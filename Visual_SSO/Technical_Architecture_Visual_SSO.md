...
>
> **The consent screen is where the sign-in ends, and that is a good outcome rather than a tolerated one.** It displays the pupil's own name and email immediately before the account is committed, making it an identity check at exactly the right moment. **This bears on G2** — *a pupil cannot sign in as a classmate*, which PRD §4 records as v1's largest defect. If Aisyah is looking at Tan Wei Ming's name she stops, and a teacher in the room can see it too. The picture password guards that failure at the start of the flow; this guards it at the end, with the account's real identity on screen in plain language.
>
> **The launcher must not click Continue, for two independent reasons.** First, it is a consent dialog — a child granting Google permission to release their name and email to an application — so automating it means the software consents on a seven-year-old's behalf, silently, every lesson. That is a different act from typing a credential the school already holds, and would rightly be raised in any review. Second, clicking it would destroy the identity check above. **Ship the click to the pupil.**
>
> **That is a smaller ask than it sounds.** It is one large, centred button. The product exists because a seven-year-old cannot type a 26-character email address; pressing a button is not that problem — and the teacher is in the room (PRD §3) and can say it once to the whole class.
>
...
>
> **Where the pupil is told to press it.** The pupil is looking at Chrome, not at the launcher, so the instruction cannot live on a launcher screen behind it. Use the **floating reset bar** (PRD §7.4) — the existing mechanism for launcher UI that coexists with Chrome. It shows one line on detecting the consent page and drops it once the destination loads. The topmost overlay must be down by then; injection is finished and nothing further is typed.
>
> **Do not name the button by its English label.** Google localises it — `Continue` on an English account, `Teruskan` on a Malay one — so *"Tekan Continue"* is wrong for some pupils and confusing for the rest. Identify it by position and colour, and use the line to prompt the identity check rather than hoping the pupil notices it: *"Lihat nama kamu. Kalau betul, tekan butang biru di bawah."*
>
